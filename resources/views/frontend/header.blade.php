...
<div class="navbar navbar-expand-md header">
		<div class="container">
			<div class="logo_container">
				<a href="{{url('/')}}"><img src="{{asset('images/logoWEB/MCK.png')}}" alt="LOGO ĐỒNG HỒ XƯA"></a> ĐỒNG HỒ MCK</>
			</div>
			...
			<div class="collapse navbar-collapse menu_header">
				...
				<ul class="navbar-nav ml-auto">
					<form class="form-inline" style="margin: auto;" method="get" action="{{url('/product')}}">
						<div class="input-group" style="min-width: 280px">
							...
							<div class="input-group-append">
			   					<button class="btn bg-warning" type="submit"><i class="fas fa-search"></i></button>
			   				</div>
						</div>
					</form>						
				    ...
				    <a class="text-white btn bg-warning button-mini-cart {{(request()->is(['cart','order'])) ? 'd-none' : '' }}"><i class="fas fa-shopping-cart"></i> Giỏ hàng <span class="badge badge-light total-quantity">{{session()->has('cart') ? session('cart')->getTotalQuantity():''}}</span></a>
