	...
	<div class="col-md-9">
		...
		<div class="row">
			<div class="col">
				...
				<div class="card-header bg-info text-uppercase text-white"><i class="fas fa-spinner"></i><b> Sản phẩm mới</b></div>
								...
								<div class="card-img-top embed-responsive embed-responsive-1by1">
							  		...
							  		<div class="button-product">
									    ...
									    @if($item->quantity_in_stock>0)
										    ...
										    <a class="btn btn-warning add-cart{{$item->id}}" href="javascript:void(0);"><i class='fas fa-check'></i> Đã thêm vào giỏ</a>
										    @else
						...
						<!-- sản phẩm -->
							<div class="card product">
								<div class="card-img-top embed-responsive embed-responsive-1by1">
							  		<img src="{{asset('images/product/'.$item->images[0]->name)}}" alt="ảnh{{ $item->name}}" class="embed-responsive-item">
							  		<div class="button-product">
									    <a href="{{url('/product/detail/'.$item->id)}}" class="btn btn-light text-secondary"><i class="fas fa-info-circle"></i> Chi tiết</a>
									    @if($item->quantity_in_stock>0)
										    @if($cart!=null&&array_key_exists($item->id,$cart->getListCartItem()))
										    <a class="btn btn-warning add-cart{{$item->id}}" href="javascript:void(0);"><i class='fas fa-check'></i> Đã thêm vào giỏ</a>
										    @else
